...
def save_cropped_result(outpath, test_label_filenames_list, img_path):
  for filename in test_label_filenames_list:
    pred = cv2.imread(outpath + filename, cv2.IMREAD_GRAYSCALE)
    ...
    if not cv2.imwrite(outpath + "cropped/" + filename, img):
      raise Exception("Could not write images")



def save_results(np_array, color_space, outpath, test_label_filenames_list, img_path):
    i = 0
    for filename in test_label_filenames_list:
        ...
        pred = pred * 255
        if not cv2.imwrite(outpath + filename, pred):
          raise Exception("Could not write image")       
        i += 1
    
    save_cropped_result(outpath, test_label_filenames_list, img_path)
